...
#### 5 марта — Версия 1.0.8
- 🗑️ **Автоматический бэкап рецептов прикормок**
  - При каждом сохранении рецептов создаётся бэкап в папке `backups/`
  - Хранится 5 последних бэкапов
  - Старые бэкапы автоматически удаляются
- ↩️ **Восстановление из бэкапа**
  - Кнопка «Восстановить из бэкапа» в панели рецептов
  - Окно выбора из 5 последних бэкапов
  - Подтверждение перед восстановлением
- 💾 **Сохранение копии рецептов**
  - Кнопка «Сохранить копию рецептов в файл»
  - Экспорт в JSON-файл с timestamp в имени
- 📍 **Копирование координат точки лова**
  - Клик по блоку с координатами в карточке точки лова
  - Копирование в буфер обмена в формате «X Y»
  - ToolTip с подсказкой
- 🎨 **Увеличен шрифт заголовков групп карт**
  - «Обычные локации» и «Локации DLC»: 14 → 16
- 🔧 **Исправление отображения кнопки «Восстановить»**
  - Текст кнопки теперь белый и центрирован
  - Кнопка растягивается по содержимому
- 🛡️ **Защита папки backups при автообновлении**
  - Папка `backups/` автоматически сохраняется при обновлении
  - Восстановление после обновления с проверкой ошибок
  - Откат изменений при ошибке копирования
- 📐 **Исправление отображения карточек рецептов**
  - Карточки рецептов расположены плотно в 2 колонки
  - Уменьшены отступы между карточками
  - Использован WrapPanel вместо UniformGrid
- 🐛 **Исправление ошибки NullReferenceException**
  - Исправлена ошибка при сохранении нового рецепта
  - Добавлена нормализация рецептов после загрузки из JSON
  - Восстановление CurrentRecipe после перестройки коллекции

#### 4 марта
- ✏️ **Редактирование карточки точки лова**
  - Замена названия водоёма на координаты
  - Координаты по центру между кнопками «Карта» и «Удалить»
- 🎨 **Исправление цветов рамок рецептов**
  - Стили `RecipeBorder*` используют цвета из тем (`RecipeNormalBorderColor` и др.)
  - Корректное переключение цветов при смене темы

#### 2 марта — Версия 1.0.7
- 🛡️ **Защита пользовательских данных при автообновлении**
  - Автоматическое резервирование `CatchPoints_Local.json` (точки лова)
  - Автоматическое резервирование `BaitRecipes.json` (рецепты прикормок)
  - Восстановление файлов после обновления
  - Проверка ошибок с откатом при неудаче
- 📝 **Обновлена документация**
  - `BUILD_RELEASE.md` — инструкция по сборке релиза
  - Исключение пользовательских файлов из ZIP-архива
- 🔧 **Технические улучшения**
  - Нормализация пути `APP_DIR` в update.bat
  - `.gitignore` для `CatchPoints_Local.json` и `BaitRecipes.json`
